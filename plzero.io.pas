...
{obtained from www.moorecad.com/standardpascal}
{pl/0 compiler with code generation}
label 99;
const norw = 11;     {no. of reserved words}
   txmax = 100;      {length of identifier table}
   kmax = 8;         {max. no. of digits in numbers, changed from nmax BPW}
   al = 10;          {length of identifiers}
   {amax = 2047;}    {maximum address-removed BPW}
   nmax = 99999999;  {maximum value allowed in source program BPW}
   levmax = 7;       {maximum depth of block nesting}
   cxmax = 5000;     {size of code array}
type symbol =
   (nul,ident,number,plus,minus,times,slash,oddsym,
    ...
    object1 = (constant,varible,proc,inp1,outp1);
    symset = set of symbol;
    fct = (lit,opr,lod,sto,cal,int,jmp,jpc,inp,outp);   {functions}

{  BPW - make all integers long, avoids problems 
   like unreachable code for errors }
    integer=longint;
...
procedure getsym;
   ...
   begin if cc = ll then
      begin if eof(input) then
                 begin writeln(' program incomplete'); {goto 99} halt(1)
                 end;
...
begin if cx > cxmax then
           begin writeln(' program too long'); {goto 99} halt(1)
           end;
   with code[cx] do
      begin f := x; l := y; a := z
      end;
...
procedure block(lev,tx: integer; fsys: symset);
   ...
   begin {enter object1 into table}
      tx := tx + 1;
      with table[tx] do
      begin name := id; kind := k;
         case k of
         constant: begin if num > nmax then {BPW - shouldn't happen!}
   ...
   begin {statement}
      ...
      begin i := position(id);
         ...
         if table[i].kind <> varible then
            begin 
            if id = 'out       ' then 
              begin getsym; if sym = becomes then getsym else error(13);
              expression(fsys); gen(outp,0,0);
              end
            else error(12); i := 0
            end
         else begin
...
begin writeln(' start pl/0');
   ...
   repeat i := code[p]; p := p + 1;
      ...
      inp: begin t := t + 1; write('? '); readln(s[t])
           end;
      ...
      sto: begin s[base(l)+a] := s[t]; {write('! '); writeln(s[t]);} t := t - 1
           end;
      outp: begin write('! '); writeln(s[t]); t := t - 1
           end;
...
begin {main program}
   ...
   block(0, 2, [period]+declbegsys+statbegsys);
